...
export const businessResetPassword = async (email) => {
    try {
        // Check if business user exists first
        const businessData = await getBusinessUserFromFirestore(email);
        if (!businessData) {
...
export const validateBusinessEmailPhone = async (email, phone) => {
    try {
        ...
        if (businessData.phone !== formattedPhone) {
