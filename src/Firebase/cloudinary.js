...
export const uploadToCloudinary = async (file, folder = '', public_id = '') => {
    ...
    if (hasEmbeddedFolder) {
        ...
        console.log('Using public_id with embedded path:', public_id);
    } else if (formattedFolder) {
        // Use separate folder parameter and public_id parameter (standard approach)
        formData.append('folder', formattedFolder);
        formData.append('public_id', public_id);
        console.log('Using folder + public_id:', { folder: formattedFolder, public_id });
    } else {
